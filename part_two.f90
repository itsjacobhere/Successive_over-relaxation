module vars
    ...
    real(wp) :: kx, ky, dx= 100, dy = 100, k, Q
...
program well_analysis
    ...
    implicit none
    integer :: i, j, c
    real(wp), dimension(:,:), allocatable :: u, f
    ...
    write(*,*)"------------------------------------------------------------------------------------"
    write(*,*) ""
    write(*,*) "Do you want to perform the sensitivity analysis on k or w? or do you want to run part2"
    write(*,*) 
    read(*,*) answer
    !sensitivity on iterations vs relaxation factor
    !sensitivity on hydraulic conductiviy vs head at each node
    !iterate Q until the difference between the final U and the initial U is equal to 4m
    !if( u+f==4) then 
    c=100
    do c = 50,150, 10
        if (answer=="no" .or. answer=="part2") then
            output = "well_analysis.csv" 
            go to 100
        end if
        ...
        !Q = 400
        satthickness = 60
        transmissivity = k*satthickness !(m2/day)
        if (answer=="k") then
            k = k*((real(c))/100)
