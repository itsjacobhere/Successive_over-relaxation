...
program well_analysis
    ...
    do 
        ...
        f(4,13) = (Q)/(dx*dy) !not sure if these should be negative because they are in SOR algo
        f(5,13) = (Q)/(dx*dy)
        f(6,13) = (Q)/(dx*dy)
        f(7,13) = (Q)/(dx*dy)
        f(3,15) = (Q)/(dx*dy)
        ...
        do i = 4, 7 !range of the gravel pit square
            do j = 14,18
                if (0 < (diff(i,j) - 4.0) .and. (diff(i,j) - 4.0) < 0.0001) then !if the diff array is 4 or more at each node, success
                ...
                !else if (diff(i,j) > 4) then ! dont need this because diff must be greater than 4
                    ...
                    Q = Q+ 0.0065 !increase the flow to make sure each node is at least 4 less
                    !write(*,*) "Q= ",Q
